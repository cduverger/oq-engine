...
def prefilter(srcs, srcfilter, count_events, monitor):
    """
    :returns: a dict src_group_id -> sources
    """
    dic = groupby(srcfilter.filter(srcs), src_group_id)
    if count_events:
        for grp_id in dic:
            for src in dic[grp_id]:
                src.num_events  # cached_property
    return dic
